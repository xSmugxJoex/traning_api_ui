...
test("Несколько кнопок и модальное окно", async ({page}) => {
  ...
  const btns = getEnumValues(s.Btn);
  ...
  await expect(page.locator(s.Btn.Primary)).toHaveCSS("background-color", Color.Btn.Primary);
  await page.locator(s.Btn.Primary).click();
  // await page.locator(s.ClassAttr.Btn.Primary).click();
  await expect(page.locator(s.Btn.Primary)).toHaveCSS("border-color", Color.BtnBorderColor.BorderPrimary);


  await expect(page.locator(s.Btn.Warning)).toHaveCSS("background-color", Color.Btn.Warning);
  await page.locator(s.Btn.Warning).click();
  await expect(page.locator(s.Btn.Warning)).toHaveCSS("border-color", Color.BtnBorderColor.BorderWarning);
  ...
  await expect(page.locator(s.Btn.Success)).toHaveCSS("border-color", Color.BtnBorderColor.BorderSuccess);
})